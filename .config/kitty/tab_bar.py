...
ACTIVE_INDEX_COLOR = Color(250, 179, 135)
PASSIVE_INDEX_COLOR = Color(137, 180, 250)

PASSIVE_BG_COLOR = Color(49, 50, 68)

ICON_COLOR = Color(166, 227, 161)
...
def _draw_element(
    ...
    if accented:
        ...
        icon_bg = colors["passive_bg"]
        idx_bg = colors["active_idx"]
        icon_col = colors["icon_col"]
    else:
        text_bg = colors["passive_bg"]
        idx_bg = colors["passive_idx"]
        icon_bg = colors["passive_bg"]
        icon_col = colors["icon_col"]
    ...
    if not icon:
        ...
        components.append((f"{index}{icon_padding}", colors["bg"], idx_bg))
    ...
    if icon:
        icon_padding = PADDING if title != "" else ""
        components.append((f"{icon}{icon_padding}", text_bg, icon_col))
        components.append((PADDING, text_bg, icon_bg))

    if not icon:
        # Title
        components.append((title, text_fg, text_bg))
    else:
        components.append((title, text_fg, icon_bg))
    ...
    if not icon:
        components.append((RIGHT_SEP, text_bg, colors["bg"]))
    else:
        components.append((RIGHT_SEP, icon_bg, colors["bg"]))
    # Inter-tab soft separator
    if soft_sep:
        components.append((soft_sep, colors["bg"], colors["bg"]))
...
def draw_tab(
    ...
    colors["active_idx"] = as_rgb(color_as_int(draw_data.active_bg))
    colors["passive_idx"] = as_rgb(color_as_int(PASSIVE_INDEX_COLOR))

    colors["passive_bg"] = as_rgb(color_as_int(PASSIVE_BG_COLOR))

    colors["icon_col"] = as_rgb(color_as_int(ICON_COLOR))
